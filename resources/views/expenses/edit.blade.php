...
@section('content')
  ...
  <div class="mt-8 max-w-2xl">
    <x-card class="p-8">
      <form method="POST" action="{{ url('/expenses/' . ($expense['id'] ?? 1)) }}" class="space-y-4">
        @csrf
        @method('PUT')

        <div>
          <label class="block text-sm font-medium text-black-600">Titre</label>
          <input name="title" value="{{ $expense['title'] ?? '' }}" required class="mt-1 w-full rounded-lg border border-gray-100 px-3 py-2 outline-none focus:border-orange-500" />
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-black-600">Montant</label>
            <input name="amount" type="number" step="0.01" value="{{ $expense['amount'] ?? '' }}" required class="mt-1 w-full rounded-lg border border-gray-100 px-3 py-2 outline-none focus:border-orange-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-black-600">Date</label>
            <input name="date" type="date" value="{{ $expense['date'] ?? '' }}" required class="mt-1 w-full rounded-lg border border-gray-100 px-3 py-2 outline-none focus:border-orange-500" />
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select name="payer_id" class="mt-1 w-full rounded-lg border border-gray-100 px-3 py-2 outline-none focus:border-orange-500">
